...
function runTests(testObj) {
  ...
  if (testObj.startUnit != null) {
    __testLog__ = document.createElement('div');
    try {
      setup_args = testObj.startUnit();
  ...
  for (var i in testObj) {
    ...
    try {
      ...
      if (testObj.setup != null)
        args = testObj.setup.apply(testObj, setup_args);
      testObj[i].apply(testObj, args);
      if (testObj.teardown != null)
        testObj.teardown.apply(testObj, args);
    }
    catch (e) {
  ...
  if (testObj.endUnit != null) {
    __testLog__ = document.createElement('div');
    try {
      testObj.endUnit.apply(testObj, setup_args);
      if (__testLog__.childNodes.length > 0)
        log.appendChild(__testLog__);
    } catch(e) {
...
qc = function(argGens, predicate, count) {
  ...
  try {
    for (var i=0; i<count; i++) {
      ...
      if (!rv) throw({predicate: predicate, args: args});
    }
  } catch(e) {
    throw({predicate: predicate, args: args, exception: e});
  }
  return true;
};
